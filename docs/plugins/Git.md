...
A plugin that commits and pushes release-related changes to the remote repository. It can be configured to include specific files (like a changelog or build artifacts) in a release commit.
...
This step pushes changes to the remote repository. It is also skipped in `DryRun` mode.
...
2.  **`Publish`**: The plugin pushes the new commit to your branch.
